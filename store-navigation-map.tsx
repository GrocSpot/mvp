...
import { Navigation, Clock, ChevronDown, RotateCcw, Check, ZoomIn, ZoomOut, HelpCircle } from "lucide-react"
...
export default function StoreNavigationMap() {
  const [selectedStore] = useState("Whole Foods Market - Downtown")
  const [estimatedTime] = useState("12 min")
  const [zoomLevel, setZoomLevel] = useState(1)

  const [shoppingItems, setShoppingItems] = useState<ShoppingItem[]>([
    { id: "1", name: "Organic Bananas", aisle: "A1", section: "Produce", found: false, x: 15, y: 20 },
    { id: "2", name: "Whole Milk", aisle: "B3", section: "Dairy", found: true, x: 45, y: 60 },
    { id: "3", name: "Sourdough Bread", aisle: "C2", section: "Bakery", found: false, x: 75, y: 35 },
    { id: "4", name: "Ground Coffee", aisle: "D4", section: "Beverages", found: false, x: 25, y: 80 },
    { id: "5", name: "Greek Yogurt", aisle: "B3", section: "Dairy", found: false, x: 50, y: 60 },
    { id: "6", name: "Chicken Breast", aisle: "E1", section: "Meat", found: false, x: 80, y: 75 },
  ])

  const toggleItemFound = (id: string) => {
    setShoppingItems((items) => items.map((item) => (item.id === id ? { ...item, found: !item.found } : item)))
  }
            ...
            {shoppingItems.map((item) => (
              <g key={item.id}>
                ...
                <text x={item.x} y={item.y + 0.5} textAnchor="middle" fontSize="1.5" fill="#ffffff" fontWeight="600">
                  {item.found ? "✓" : shoppingItems.indexOf(item) + 1}
                </text>
              </g>
            ))}
